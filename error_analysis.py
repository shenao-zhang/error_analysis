...
class DPOInference:
    ...
    def tokenize_row(self, feature) -> Dict:
        ...
        """
        batch = {}
        chosen_message = feature["chosen"][-1:]
        rejected_message = feature["rejected"][-1:]
        prompt = feature["prompt"]
        #chosen = feature["chosen"]  # modified from source
        #rejected = feature["rejected"]  # modified from source
        chosen = self.tokenizer.apply_chat_template(chosen_message, tokenize=False)
        rejected = self.tokenizer.apply_chat_template(rejected_message, tokenize=False)
